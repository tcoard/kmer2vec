...
def main():
    num_to_save = 10
    final = dict()
    with open("matches_formatted.json", "r") as match_file, open("matches_compared.json", "w") as out_file:
            ...
            with open(f"blast_files/{fname}_1.json", "r") as f:
                ...
                for i, match in enumerate(seq["matches"]):
                    ...
                    if i < num_to_save:
                        # break
                        seq_id = match["id"]
                        final[orig_seq_id]["our_matches"].append(seq_id)
